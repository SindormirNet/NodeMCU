...
const String html ="<body><header><h1>Lampara RGB</h1><h2>con ESP8266</h2></header><section><form class='' action='' method='post'><label>Color e intensidad</label><input type='color' name = 'Colores' class = 'selectorColor' placeholder='Color'><input type='range' name = 'intensidad'><input type='submit'></form></section><footer><img src='https://sindormir.net/sites/sindormir.net/files/logo-sindormir-transparente-ajustado.png'</footer></body></HTML>";
...
void miWeb(){
  ...
  iluminacion(dameColores(colores)[0],dameColores(colores)[1],dameColores(colores)[2],255);

  //Comprobamos los que estamos enviando a cada led
  Serial.println("Enviado al led R: "+String(dameColores(colores)[0]));
  Serial.println("Enviado al led G:"+String(dameColores(colores)[1]));
  Serial.println("Enviado al led B: "+dameColores(colores)[2]);
}
